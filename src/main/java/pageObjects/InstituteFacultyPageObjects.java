...
public class InstituteFacultyPageObjects extends TestBase {
	...
	public String createInstituteFaculty(HashMap<String, String> testData) throws Exception {
		System.out.println(driver);
		clickCreateNewFaculty();
		Thread.sleep(3000);
		...
		commonMethods.selectDropdownOption(create_subject,
				testData.get(CommonConstants.INSTITUTE_FACULTY_SUBJECT).toString());
		...
		create_password.sendKeys(testData.get(CommonConstants.INSTITUTE_FACULTY_PASSWORD).toString());
		create_confirmPassword.sendKeys(testData.get(CommonConstants.INSTITUTE_FACULTY_CONFIRM_PASSWORD).toString());

		System.out.println(
				"The institute name is " + testData.get(CommonConstants.INSTITUTE_FACULTY_USER_NAME).toString());
		System.out
				.println("The institute email is " + testData.get(CommonConstants.INSTITUTE_FACULTY_EMAIL).toString());

		createNewFacultyInThePopUp.click();
		
		//Click OK from the success pop-up
		clickOk.click();		
